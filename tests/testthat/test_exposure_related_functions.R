...
test_that("plot_exposure_internal function", {
  ...
  # xlim and legend.x
  out4 <-
    plot_exposure_internal(sort_exposure(exposure[ , 3:6]), xlim = c(0, 25),
...
test_that("plot_exposure function", {
  ...
  on.exit(par(old.par))
  out <- plot_exposure(exposure = sort_exposure(exposure[, 1:43]),
  ...
  # Only plot the first 30 samples
  par(old.par)
  out1 <- plot_exposure(exposure = sort_exposure(exposure[, 1:30]),  # Test a trick edge case
                       main = "test1", cex.main = 0.8, cex.legend = 0.45)
  expect_equal(out1$plot.success, TRUE)

  # Only plot one sample
  out2 <- plot_exposure(exposure = sort_exposure(exposure[, 1, drop = FALSE]),
                       samples.per.line = 4, cex.legend = 0.45,
                       legend.x = 1.5)
  expect_equal(out2$plot.success, TRUE)

  # Only plot selected samples
  out3 <- plot_exposure(exposure = sort_exposure(exposure[, 3:6]),
                       samples.per.line = 4, cex.legend = 0.45,
                       legend.x = 4.5)
  expect_equal(out3$plot.success, TRUE)
  ...
  out4 <- plot_exposure(exposure = sort_exposure(exposure[, 1:43 ]),
  ...
  out5 <- plot_exposure(exposure = sort_exposure(exposure[1:4, 3:6]),
                       samples.per.line = 4, plot.proportion = TRUE,
                       col = c("red", "blue"), cex.legend = 0.45,
                       legend.x = 4.5)
  expect_equal(out5$plot.success, TRUE)

  out6 <- plot_exposure(exposure = sort_exposure(exposure[, 3:6]),
                       samples.per.line = 4, plot.proportion = TRUE,
                       col = c("red", "blue"), cex.legend = 0.45,
                       legend.x = 4.5)
  expect_equal(out6$plot.success, TRUE)
...
test_that("plot_exposure_to_pdf function", {
  ...
  out0 <- plot_exposure_to_pdf(exposure = sqrt(sort_exposure(exposure)),
  ...
  out0.1 <- plot_exposure_to_pdf(exposure = sqrt(sort_exposure(exposure)),
  ...
  out1 <- plot_exposure_to_pdf(exposure = sort_exposure(exposure),
                            file = file.path(tempdir(), "test1.pdf"),
                            ylim = c(0, max(colSums(exposure))))
  expect_equal(out1$plot.success, TRUE)

  # Only plot the first 30 samples
  out2 <- plot_exposure_to_pdf(exposure = sort_exposure(exposure[, 1:30]),
  ...
  out3 <- plot_exposure_to_pdf(exposure = sort_exposure(exposure[, 1:10]),
  ...
  # Only plot selected samples with selected signatures
  out4 <- plot_exposure_to_pdf(exposure = exposure[1:5, 1:15],
  ...
  # Plot exposure proportions rather than counts
  out5 <- plot_exposure_to_pdf(exposure = sort_exposure(exposure),
                    file = file.path(tempdir(), "test5.pdf"),
                    plot.proportion = TRUE)
  expect_equal(out5$plot.success, TRUE)

  # Only plot selected samples with selected signatures
  out6 <- plot_exposure_to_pdf(exposure = exposure[1:5, 1:15],
